...
int blake2xs_final(blake2xs_state* S, void* out, size_t outlen) {
  ...
  for (i = 0; outlen > 0; ++i) {
    const size_t block_size =
        (outlen < BLAKE2S_OUTBYTES) ? outlen : BLAKE2S_OUTBYTES;
    /* Initialize state */
    ...
    if (blake2s_final(C, (uint8_t*)out + i * BLAKE2S_OUTBYTES, block_size) <
        0) {
      return -1;
    }
    outlen -= block_size;
...
int main(void) {
  ...
  for (outlen = 1; outlen <= BLAKE2_KAT_LENGTH; ++outlen) {
    ...
    if (blake2xs(hash, outlen, buf, BLAKE2_KAT_LENGTH, key, BLAKE2S_KEYBYTES) <
        0) {
      goto fail;
    }

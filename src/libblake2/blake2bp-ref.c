...
static int blake2bp_init_leaf(blake2b_state* S,
                              size_t outlen,
                              size_t keylen,
                              uint64_t offset) {
  blake2b_param P[1];
...
int blake2bp_init(blake2bp_state* S, size_t outlen) {
  ...
  for (i = 0; i < PARALLELISM_DEGREE; ++i)
    if (blake2bp_init_leaf(S->S[i], outlen, 0, i) < 0)
      return -1;
...
int blake2bp_init_key(blake2bp_state* S,
  ...
  for (i = 0; i < PARALLELISM_DEGREE; ++i)
    if (blake2bp_init_leaf(S->S[i], outlen, keylen, i) < 0)
      return -1;
...
int blake2bp_update(blake2bp_state* S, const void* pin, size_t inlen) {
  ...
  if (left && inlen >= fill) {
    ...
    for (i = 0; i < PARALLELISM_DEGREE; ++i)
      blake2b_update(S->S[i], S->buf + i * BLAKE2B_BLOCKBYTES,
                     BLAKE2B_BLOCKBYTES);
...
int blake2bp_final(blake2bp_state* S, void* out, size_t outlen) {
  ...
  for (i = 0; i < PARALLELISM_DEGREE; ++i) {
    if (S->buflen > i * BLAKE2B_BLOCKBYTES) {
      ...
      if (left > BLAKE2B_BLOCKBYTES)
        left = BLAKE2B_BLOCKBYTES;
...
int main(void) {
  ...
  for (step = 1; step < BLAKE2B_BLOCKBYTES; ++step) {
    for (i = 0; i < BLAKE2_KAT_LENGTH; ++i) {
      ...
      if ((err = blake2bp_init_key(&S, BLAKE2B_OUTBYTES, key,
                                   BLAKE2B_KEYBYTES)) < 0) {
        goto fail;
      }

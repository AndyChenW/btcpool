...
static const int cacheSize = 42;  //"Answer to the Ultimate Question of Life,
...
uint8_t* GpuTs(uint8_t blockheader[32], uint8_t seed[32]) {
  ...
  // auto d6 = s7 - s6;
  // std::cout << "d6 duration: " <<
  // chrono::duration_cast<chrono::microseconds>(d6).count() << " micros\n";
  ...
  if (seedCache.size() > cacheSize) {
    for (map<vector<uint8_t>, BytomMatListGpu*>::iterator it =
             seedCache.begin();
         it != seedCache.end(); ++it) {
      delete it->second;
    }

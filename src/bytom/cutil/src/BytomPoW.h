...
inline void sortLtcMemoryForMatrixCopy(LTCMemory& ltcMem,
...
struct Mat256x256i8 {
  ...
  Mat256x256i8(const Mat256x256i8& other) { this->copyFrom(other); }
  ...
  void copyFromEven(LTCMemory& ltcMem) { copyFrom_helper(ltcMem, 0); }

  void copyFromOdd(LTCMemory& ltcMem) { copyFrom_helper(ltcMem, 1); }
...
struct Arr256x64i32 {
  ...
  uint8_t* d0RawPtr() { return (uint8_t*)(d[0]); }
...
struct BytomMatList8 {
  ...
  Mat256x256i8 at(int i) { return matVec[i]; }

  BytomMatList8() { matVec = new Mat256x256i8[256]; }

  ~BytomMatList8() { delete[] matVec; }
...
struct BytomMatListGpu {
  int8_t* matVecGpu;
  int8_t* at(int i) { return &matVecGpu[i * 256 * 256]; }
  BytomMatListGpu() {
    int8_t* devPtr_i8;
    cudaMalloc((void**)&devPtr_i8, 256 * 256 * 256 * sizeof(*devPtr_i8));
    matVecGpu = devPtr_i8;
  }
  ~BytomMatListGpu() { cudaFree(matVecGpu); }
};
...
extern void initMatVecGpu(BytomMatListGpu* matListGpu_int8,
                          BytomMatList8* matList_int8);
extern void core_mineBytom_gpu(std::vector<uint8_t> fourSeq[4],
...
inline void iter_mineBytom(const uint8_t* fixedMessage,
  ...
  for (int k = 0; k < 4; k++) {  // The k-loop
    ...
    rhash_sha3_update(&ctx, fixedMessage + (len * k / 4),
                      len / 4);  //分四轮消耗掉fixedMessage

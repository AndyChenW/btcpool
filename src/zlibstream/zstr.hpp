...
class Exception : public std::exception {
 public:
  Exception(z_stream* zstrm_p, int ret) : _msg("zlib: ") {
    switch (ret) {
      case Z_STREAM_ERROR:
        _msg += "Z_STREAM_ERROR: ";
        break;
...
class ostreambuf : public std::streambuf {
 public:
  ...
  int deflate_loop(int flush) {
    while (true) {
      zstrm_p->next_out =
          reinterpret_cast<decltype(zstrm_p->next_out)>(out_buff);
  ...
  virtual std::streambuf::int_type overflow(
    ...
    setp(in_buff, in_buff + buff_size);
    return traits_type::eq_int_type(c, traits_type::eof()) ? traits_type::eof()
                                                           : sputc(c);
  }
  virtual int sync() {
...
class ostream : public std::ostream {
 public:
  ostream(std::ostream& os, int level = Z_DEFAULT_COMPRESSION)
      : std::ostream(
            new ostreambuf(os.rdbuf(), ostreambuf::default_buff_size, level)) {
    exceptions(std::ios_base::badbit);
  }
 ...
 public:
  explicit ofstream(const std::string& filename,
      ...
      : detail::strict_fstream_holder<strict_fstream::ofstream>(
        ...
        std::ostream(
            new ostreambuf(_fs.rdbuf(), ostreambuf::default_buff_size, level)) {
    exceptions(std::ios_base::badbit);
  }
  ofstream& flush() {
    std::ostream::flush();

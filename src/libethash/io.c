...
enum ethash_io_rc ethash_io_prepare(char const* dirname,
  ...
  char* tmpfile =
      ethash_io_create_filename(dirname, mutable_name, strlen(mutable_name));
  if (!tmpfile) {
  ...
  if (!force_create) {
    ...
    if (f) {
      ...
      // compare the magic number, no need to care about endianess since it's
      // local
      uint64_t magic_num;
  ...
  if (ethash_fseek(f, file_size + ETHASH_DAG_MAGIC_NUM_SIZE - 1, SEEK_SET) !=
      0) {
    fclose(f);
    ETHASH_CRITICAL(
        "Could not seek to the end of DAG file: \"%s\". Insufficient space?",
        tmpfile);
    goto free_memo;
  }
  if (fputc('\n', f) == EOF) {
    fclose(f);
    ETHASH_CRITICAL(
        "Could not write in the end of DAG file: \"%s\". Insufficient space?",
        tmpfile);
    goto free_memo;
  }
  if (fflush(f) != 0) {
    fclose(f);
    ETHASH_CRITICAL(
        "Could not flush at end of DAG file: \"%s\". Insufficient space?",
        tmpfile);
    goto free_memo;
  }

  ...
  for (unsigned i = 0; i != ETHASH_ACCESSES; ++i) {
    ...
    for (unsigned n = 0; n != MIX_NODES; ++n) {
...
#elif defined(__MIC__)
      {
        ...
        //	Each vector register (zmm) can store sixteen 32-bit integer
        //numbers
...
// The regression was introduced in GCC 4.8.4, fixed in GCC 5.0.0 and backported
// to GCC 4.9.3 but
// never to the GCC 4.8.x line.
//
// See https://gcc.gnu.org/bugzilla/show_bug.cgi?id=56273
//
// This regression is affecting Debian Jesse (8.5) builds of cpp-ethereum (GCC
// 4.9.2) and also
// manifests in the doublethinkco armel v5 cross-builds, which use crosstool-ng
// and resulting
// in the use of GCC 4.8.4.  The Tizen runtime wants an even older GLIBC version
// - the one from
// GCC 4.6.0!
...
#pragma GCC diagnostic pop
  ...
  // final Keccak hash
  SHA3_256(&ret->result, s_mix->bytes,
           64 + 32);  // Keccak-256(s + compressed_mix)
  return true;
}
...
static bool ethash_mmap(struct ethash_full* ret, FILE* f) {
  ...
  mmapped_data = mmap(NULL, (size_t)ret->file_size + ETHASH_DAG_MAGIC_NUM_SIZE,
...
ethash_full_t ethash_full_new_internal(char const* dirname,
  ...
  enum ethash_io_rc err =
      ethash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, false);
  if (err == ETHASH_IO_FAIL)
    goto fail_free_full;

  if (err == ETHASH_IO_MEMO_SIZE_MISMATCH) {
    // if a DAG of same filename but unexpected size is found, silently force
    // new file creation
    if (ethash_io_prepare(dirname, seed_hash, &f, (size_t)full_size, true) !=
        ETHASH_IO_MEMO_MISMATCH) {
      ETHASH_CRITICAL(
          "Could not recreate DAG file after finding existing DAG with "
          "unexpected size.");
      goto fail_free_full;
    }
  ...
  if (err == ETHASH_IO_MEMO_MISMATCH || err == ETHASH_IO_MEMO_MATCH) {
    ...
    if (err == ETHASH_IO_MEMO_MATCH) {
      ...
      uint32_t const countnodes =
          (uint32_t)((size_t)ret->file_size / sizeof(node));
  ...
  // after the DAG has been filled then we finalize it by writting the magic
...
fail_free_full_data:
  // could check that munmap(..) == 0 but even if it did not can't really do
  // anything here
...
ethash_return_value_t ethash_full_compute(ethash_full_t full,
  ...
  ethash_return_value_t ret;
  ret.success = true;
  if (!ethash_hash(&ret, (node const*)full->data, NULL, full->file_size,
                   header_hash, nonce)) {
    ret.success = false;
  }
  return ret;

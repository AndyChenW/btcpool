...
static bool ParsePrechecks(const std::string& str) {
  if (str.empty())  // No empty string allowed
    return false;
  if (str.size() >= 1 &&
      (IsSpace(str[0]) || IsSpace(str[str.size() - 1])))  // No padding allowed
    return false;
...
bool ParseInt32(const std::string& str, int32_t* out) {
  ...
  if (out)
    *out = (int32_t)n;
  // Note that strtol returns a *long int*, so even if strtol doesn't report an
  // over/underflow
...
bool ParseUInt32(const std::string& str, uint32_t* out) {
  ...
  if (str.size() >= 1 && str[0] == '-')  // Reject negative values,
                                         // unfortunately strtoul accepts these
                                         // by default if they fit in the range
    return false;
  char* endp = nullptr;
...
bool ParseUInt64(const std::string& str, uint64_t* out) {
  if (!ParsePrechecks(str))
    return false;
  if (str.size() >= 1 && str[0] == '-')  // Reject negative values,
                                         // unfortunately strtoull accepts these
                                         // by default if they fit in the range
    return false;
  char* endp = nullptr;
  ...
  // doesn't report an over/underflow
  // we still have to check that the returned value is within the range of an
  // *uint64_t*.
  return endp && *endp == 0 && !errno &&
...
static inline bool ProcessMantissaDigit(char ch,
                                        int64_t& mantissa,
                                        int& mantissa_tzeros) {
  if (ch == '0')
    ++mantissa_tzeros;
  ...
  while (std::getline(ss, item, '/')) {
    ...
    if (pos != std::string::npos) {
      ...
      item = item.substr(
          0,
          item.size() -
              1);  // Drop the last character which is the hardened tick
    }
